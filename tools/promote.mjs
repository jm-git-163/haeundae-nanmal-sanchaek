...
const DROP = new RegExp([
  '입찰|낙찰|계약|용역|수의계약|인사발령|채용|임용|의회|조례|규칙\\s*개정|감사\\s*결과|공유재산|매각|분양',
  '특별공급|기관추천|재개발|재건축|입찰공고|낙찰자|사업자\\s*선정|위탁\\s*운영자|기간제근로자|공무직',
  '자원봉사자\\s*모집|입주자\\s*모집|취약계층지원|공시송달|영업정지|행정처분|처분\\s*(사전)?통지|청문',
  '과태료|종합건설업|건설업\\s*(등록|폐업|양도)|폐업신고|등록\\s*공고|양도·?양수|임대\\s*(공고|재공고|\\d차공고)',
  '온비드|대관\\s*공고|심의\\s*결과|위원회\\s*(구성|명단|개최|심의)|참석위원|공람|열람\\s*공고|의견\\s*청취',
  '고시\\s*(제정|개정|폐지)|위탁내용\\s*등\\s*고시|지적재조사|도시계획\\s*(시설|변경)|실시계획|지구단위계획',
  '정기검사|검사지연|미필',
  // 기관 자체 직원 채용 과정(주민과는 무관한 내부 인사 절차)
  '수련(?:생|과정)\\s*모집|합격자\\s*발표|최종\\s*합격|필기시험|서류전형|면접\\s*대상자|전형\\s*결과|채용\\s*공고|인력풀\\s*모집\\s*결과',
  // 이미 마감됐거나(신청해도 소용없음) 내부 행정·선정 결과 발표(주민이 할 일이 없는 통보성 글)
  '\\[마감\\]|접수\\s*마감|선정\\s*결과|등록\\s*결과|모집\\s*결과|심사\\s*결과\\s*(공고|안내|발표)|예산안\\s*공고|세입세출.{0,6}예산'
].join('|'));
...
const main = async () => {
  ...
  // 다음 날 자동 수집 한 번에 통째로 사라집니다. 같은 글이면 반드시 이어 붙입니다.
  const aiByKey = new Map();
  (feed.notices || []).forEach(n => {
    if (n.aiSimplified) aiByKey.set(n.source + '|' + n.title.replace(/\s/g, ''), n.aiSimplified);
  });
